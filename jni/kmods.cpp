...
int main(int argc, char *argv[]) {
    ...
    while ((c = getopt_long(argc, argv, short_options, long_options, nullptr)) != -1) {
        switch (c) {
            ...
            case 'o':
                ...
                break;
            case 'g':
                Offsets::GNames = getHexAddr(optarg);               // 这里获取了GNames的地址
                break;
            case 'u':
                Offsets::GUObjectArray = getHexAddr(optarg);        // 这里获取了GUObjectArray的地址
                break;
            case 'w':
                Offsets::GWorld = getHexAddr(optarg);                // 这里获取了GWorld的地址
                break;
            case 'n':
                isObjsDump = true;
                break;
            case 's':
                ...
                break;
            case 'a':
                isSdkDump = true;                               // Dump SDK with GUObject
                break;
            case 'b':
                isSdkDump2 = true;                              // Dump SDK with Gworld
                break;
            case 'c':
                isUE423_UE425 = true;                               
                break;
            case 'k':
                isUE425 = true;                                 // 这里的 isUE425 代表 UE4.25 以上的版本
                break;
            case 'm':
                isUE422 = true;
                break;
    ...
    if (isStrDump) {
        if (Offsets::GNames < 1) {                          // 这里检测GName的地址是否正确
