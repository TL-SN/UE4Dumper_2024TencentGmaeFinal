...
struct UObject {
    ...
    static string getName(kaddr object) {                       //  Uobject是所有对象的基类
        // printf("\ngetNameID: %d\n", getNameID(object));
...
struct UStruct {
    static kaddr getSuperClass(kaddr structz) {//UStruct*           // 已处理
        return getPtr(structz + Offsets::UStructToSuperStruct);
    }

    static kaddr getChildren(kaddr structz) {//UField*              // 已处理
        return getPtr(structz + Offsets::UStructToChildren);
    }

    static kaddr getChildProperties(kaddr structz) {//UField*       // 已处理，虽有疑问
    ...
    static string getStructClassPath(kaddr clazz) {         // 已处理
        string classname = UObject::getName(clazz);
